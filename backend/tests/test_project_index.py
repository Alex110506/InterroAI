...
from core.vector_store import collection_size, search_chunks, stored_manifest
...
async def _index(path, *, force: bool = False) -> dict:
    """Run a full indexing pass and return its final summary event."""
    summary: dict = {}
    async for event in project_index.embed_project(path, force=force):
...
async def test_a_shrunken_file_loses_the_chunks_past_its_new_end(
    ...
    monkeypatch.setattr("core.chunker._CHUNK_SIZE", 40)
    monkeypatch.setattr("core.chunker._CHUNK_OVERLAP", 0)
...
async def test_batches_already_embedded_survive_a_later_failure(
    ...
    monkeypatch.setattr(project_index, "embed_batches", one_good_batch_then_boom)
    summary = await _index(project)
...
async def test_a_failed_run_is_finished_by_the_next_one(
    ...
    monkeypatch.setattr(project_index, "embed_batches", only_the_first)
    await _index(project)

    # Recovered provider: the files that never landed are still pending.
    monkeypatch.setattr(project_index, "embed_batches", _passthrough)
    summary = await _index(project)
...
async def test_a_skipped_chunk_is_reported_not_hidden(
    ...
    monkeypatch.setattr(project_index, "embed_batches", one_fails)
    summary = await _index(project)
...
async def test_a_file_that_embedded_nothing_is_retried_on_the_next_run(
    ...
    monkeypatch.setattr(project_index, "embed_batches", fail_everything)
    await _index(project)
    assert collection_size(str(project)) == 0
...
async def test_cache_hits_are_reported(project, isolated_chroma, monkeypatch):
    ...
    monkeypatch.setattr(project_index, "embed_batches", all_cached)
    summary = await _index(project)
